...
def send_forgot_token(request, db_token_obj, clear_token):
    ...
    if asbool(threadlocal.get_current_registry().settings['datameta.logging.log_token_urls']):
        log.debug(f"Token requested; token_url={token_url}")
...
def v_forgot_api(request):
    ...
    try:
        ...
        # User not found
        log.debug(f"Recovery token requested, user unresolvable; req_email={req_email}")
    else:
        # Generate a new forgot token and send it to the user
        send_forgot_token(request, db_token_obj, clear_token)
        log.debug(f"Recovery token requested; clear_token={clear_token}")
